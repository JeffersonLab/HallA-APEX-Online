...
void replay_apex_coin(Int_t runnumber=0,Int_t numevents=0,Int_t fstEvt=0,Bool_t QuietRun = kFALSE, Bool_t OnlineReplay =kFALSE, Bool_t bPlots = kFALSE, Bool_t left = kTRUE, Bool_t right = kTRUE, Bool_t autoreplay = kFALSE, Bool_t skim = kFALSE){
  ...
  TString rootname;
  if(OnlineReplay){
    rootname = "%s/apex_online_coin_%d.root";}
  else {
    rootname = "%s/apex_coin_%d.root";
    if (skim)
...
//		gHaApps->Add(FbusRrb);
  ...
  CUTS=Form(REPLAY_DIR_PREFIX,"coinc_apex.cuts");
