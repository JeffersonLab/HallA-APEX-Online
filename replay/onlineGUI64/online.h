...
public:
  ...
  Bool_t ParseConfig();
  TString GetConfigFile() { return confFileName; };
...
public:
  ...
  void TreeDraw(const drawcommand&);
  void HistDraw(const drawcommand&);
  void MacroDraw(const drawcommand&);
  void CanvasDraw(const drawcommand&,UInt_t);
  void GraphDraw(const drawcommand&);
