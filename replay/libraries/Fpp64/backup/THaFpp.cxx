	...
	if (pulser_fpp) {
		...
		if(ana_front) {
			...
			// mkj crude track start section
				...
				zave2 = zave2/npt2;
				if ( (zave2-zave1) != 0. ) {
					vang_cr = (ave2-ave1)/(zave2-zave1);
				}
				else {vang_cr = 0.;}
				vint_cr = ave1 - vang_cr*zave1;
				ave1 = 0.;
				ave2 = 0.;
				zave1 = 0.;
				zave2 = 0.;
				...
				zave2 = zave2/npt2;
				if ( (zave2-zave1) != 0. ) {
					uang_cr = (ave2-ave1)/(zave2-zave1);
				}
				else {uang_cr = 0.;}
				uint_cr = ave1 - uang_cr*zave1;
				// mkj crude track end section
...
//ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if( (ntrack_front == 2 && align_opt == 0 && align) 
          ...
          //     chambers.
           ...
           xatz0x=track[0][8];
	   //mkj crude track start section
            xatz0f_cr=(uint_cr-vint_cr)/1.414213562;
            yatz0f_cr=(uint_cr+vint_cr)/1.414213562;
           tthetaf_cr = (uang_cr+vang_cr)/1.414213562;
           tphif_cr = (uang_cr-vang_cr)/1.414213562;
           // mkj crude track end section
...
// on the alignment correction parameters
           ...
           if (!mkjtrack){
// SD >>> NEW ROUTINE:
//	     cout << "\n front x"  << fM1Front[0][0] << setw(15) << fM1Front[1][0] << setw(15) << fM1Front[2][0] << setw(15)<< fM1Front[3][0] << setw(15)<< fM1Front[4][0] << setw(15) << fM1Front[5][0];
	     //     cout << "\n front y"  << fM1Front[0][1] << setw(15) << fM1Front[1][1] << setw(15) << fM1Front[2][1] << setw(15)<< fM1Front[3][1] << setw(15)<< fM1Front[4][1] << setw(15) << fM1Front[5][1];
	     //  cout << "\n front th"  << fM1Front[0][2] << setw(15) << fM1Front[1][2] << setw(15) << fM1Front[2][2] << setw(15)<< fM1Front[3][2] << setw(15)<< fM1Front[4][2] << setw(15) << fM1Front[5][2];
	     // cout << "\n front ph"  << fM1Front[0][3] << setw(15) << fM1Front[1][3] << setw(15) << fM1Front[2][3] << setw(15)<< fM1Front[3][3] << setw(15)<< fM1Front[4][3] << setw(15) << fM1Front[5][3];
	    // mkj crude front start section
	     temp_x=xatz0r;
	     temp_y=yatz0r;
	     temp_t=tthetar;
	     temp_p=tphir;
	     //            aligncorr_(xatz0f_cr,yatz0f_cr,tthetaf_cr,tphif_cr,
	     //                       temp_x,temp_y,temp_t,temp_p,&fM1Front[0][0],&fM1Rear[0][0]);
	    // mkj crude front end section
...
// chambers, which should be at about z=350 cm.
	   ...
	   //MKJ changes 06/14/11
           xdiff_front=(xinit+100*phiinit)-(xatz0f+100*tphif);
           ydiff_front=(yinit+100*thinit)-(yatz0f+100*tthetaf);

           x_fa=xatz0f+100*tphif;
           y_fa=yatz0f+100*tthetaf;
...
// zclose and sclose between front and rear FPP
	   ...
	   // zclose and sclose between front FPP and VDC
           a0 = pow((xatz0f-xinit),2) + pow((yatz0f-yinit),2);
           b1 = pow(tphia,2) + pow(tthetaa,2) + 1.0;
           b2 = pow(tphif,2) + pow(tthetaf,2) + 1.0;
           c1 = (xatz0f-xinit)*tphia + (yatz0f-yinit)*tthetaa;
           c2 = (xatz0f-xinit)*tphif + (yatz0f-yinit)*tthetaf;
           c3 = tphia*tphif + tthetaa*tthetaf + 1.0;
           lambda1 =  (c1*b2 - c2*c3) / (b1*b2 - c3*c3);
           lambda2 =  (c1*c3 - c2*b1) / (b1*b2 - c3*c3);
           zclose_frvdc = 0.5 * (lambda1 + lambda2);
           sclose_frvdc = sqrt(a0 + pow(lambda1,2) * b1 + pow(lambda2,2) * b2
            +2.0*(-lambda1*c1 + lambda2*c2 - lambda1*lambda2*c3));
	   // mkj crude front start section
	   // zclose and sclose between front FPP and VDC
           a0 = pow((xatz0f_cr-xinit),2) + pow((yatz0f_cr-yinit),2);
           b1 = pow(tphia,2) + pow(tthetaa,2) + 1.0;
           b2 = pow(tphif_cr,2) + pow(tthetaf_cr,2) + 1.0;
           c1 = (xatz0f_cr-xinit)*tphia + (yatz0f_cr-yinit)*tthetaa;
           c2 = (xatz0f_cr-xinit)*tphif_cr + (yatz0f_cr-yinit)*tthetaf_cr;
           c3 = tphia*tphif_cr + tthetaa*tthetaf_cr + 1.0;
           lambda1 =  (c1*b2 - c2*c3) / (b1*b2 - c3*c3);
           lambda2 =  (c1*c3 - c2*b1) / (b1*b2 - c3*c3);
           zclose_cfrvdc = 0.5 * (lambda1 + lambda2);
           sclose_cfrvdc = sqrt(a0 + pow(lambda1,2) * b1 + pow(lambda2,2) * b2
            +2.0*(-lambda1*c1 + lambda2*c2 - lambda1*lambda2*c3));
	   // mkj crude front end section

// Another method of changing to spherical coordinates
// determine theta_az, phi_az for front vs rear FPP
...
// determine theta_az, phi_az for VDC vs front FPP
           psivdc=atan(tphia*cos(atan(tthetaa)));
           psif=atan(tphif*cos(atan(tthetaf)));
           xhatf=sin(psif);
           yhatf=cos(psif)*sin(atan(tthetaf));
           zhatf=cos(psif)*cos(atan(tthetaf));

           xhat1f=xhatf*cos(psivdc)-
               yhatf*sin(atan(tthetaa))*sin(psivdc)-
               zhatf*cos(atan(tthetaa))*sin(psivdc);
           yhat1f=
               yhatf*cos(atan(tthetaa))-
               zhatf*sin(atan(tthetaa));
           zhat1f=xhatf*sin(psivdc)+
               yhatf*sin(atan(tthetaa))*cos(psivdc)+
               zhatf*cos(atan(tthetaa))*cos(psivdc);

           psisc=asin(xhat1f);
           if(cos(psisc) != 0.){ 
              thetasc=asin(yhat1f/cos(psisc));
           }
           else {
              thetasc = -1000.;
              cout << "Zero problem with psisc" << endl; 
           ...
           theta_sc_frvdc=thetasc;
           phi_sc_frvdc=phisc;
           psi_sc=atan(tan(phi_sc_frvdc)*cos(theta_sc_frvdc));
           x_sc=sin(psi_sc);
           y_sc=cos(psi_sc)*sin(theta_sc_frvdc);
           z_sc=cos(psi_sc)*cos(theta_sc_frvdc);
           r_sc=sqrt(pow(x_sc,2)+pow(y_sc,2));
           if(z_sc != 0. && y_sc != 0.) {
              theta_az_frvdc=atan(r_sc/z_sc);
              phi_az_frvdc=atan(x_sc/y_sc);
           }
           else {
              theta_az_frvdc = -1000.;
              phi_az_frvdc = -1000.;
           ...
           theta_az_frvdc=theta_az_frvdc*fg;
           phi_az_frvdc=phi_az_frvdc*fg;
           if(y_sc < 0.0) {phi_az_frvdc=phi_az_frvdc+180.0;}
           if(y_sc >= 0.0 && x_sc < 0.0) {phi_az_frvdc=phi_az_frvdc+360.0;}

           theta_sc_frvdc=theta_sc_frvdc*fg;
           phi_sc_frvdc=phi_sc_frvdc*fg;

           if (fDebug > 0) {
              cout << "Calculated front vs VDC  azimuthal angles are  " << theta_az_frvdc << " " << phi_az_frvdc << endl;
              cout << "Vertex coordinates zclose_frvdc sclose_frvdc =  " << zclose_frvdc << " " << sclose_frvdc << endl;
           }
	   // mkj crude front start section
// determine theta_az, phi_az for VDC vs crude front FPP
           psivdc=atan(tphia*cos(atan(tthetaa)));
           psif=atan(tphif_cr*cos(atan(tthetaf_cr)));
           xhatf=sin(psif);
           yhatf=cos(psif)*sin(atan(tthetaf_cr));
           zhatf=cos(psif)*cos(atan(tthetaf_cr));
           ...
           theta_sc_cfrvdc=thetasc;
           phi_sc_cfrvdc=phisc;
           psi_sc=atan(tan(phi_sc_cfrvdc)*cos(theta_sc_cfrvdc));
           x_sc=sin(psi_sc);
           y_sc=cos(psi_sc)*sin(theta_sc_cfrvdc);
           z_sc=cos(psi_sc)*cos(theta_sc_cfrvdc);
           r_sc=sqrt(pow(x_sc,2)+pow(y_sc,2));
           if(z_sc != 0. && y_sc != 0.) {
              theta_az_cfrvdc=atan(r_sc/z_sc);
              phi_az_cfrvdc=atan(x_sc/y_sc);
           }
           else {
              theta_az_cfrvdc = -1000.;
              phi_az_cfrvdc = -1000.;
              cout << "Zero problem with zsc or ysc" << endl;
           }
           theta_az_cfrvdc=theta_az_cfrvdc*fg;
           phi_az_cfrvdc=phi_az_cfrvdc*fg;
           if(y_sc < 0.0) {phi_az_cfrvdc=phi_az_cfrvdc+180.0;}
           if(y_sc >= 0.0 && x_sc < 0.0) {phi_az_cfrvdc=phi_az_cfrvdc+360.0;}

           theta_sc_cfrvdc=theta_sc_cfrvdc*fg;
           phi_sc_cfrvdc=phi_sc_cfrvdc*fg;

           if (fDebug > 0) {
              cout << "Calculated crude front vs VDC  azimuthal angles are  " << theta_az_cfrvdc << " " << phi_az_cfrvdc << endl;
              cout << "Vertex coordinates zclose_frvdc sclose_frvdc =  " << zclose_cfrvdc << " " << sclose_cfrvdc << endl;
           }
	   //mkj crude front end section
	   // start Ed Brash conetest definition
...
// decided to put a 10cm "border" around the outside, to be conservative
	...
	if((coneteste!=conetest)&&fDebug>0) {
	   ...
	   // start Ed Brash conetest definition for VDC vs front
        // 
         xf3=tphia*fZfpp[5]+xinit;
         yf3=tthetaa*fZfpp[5]+yinit;
         xr3=tphif*fZfpp[5]+xatz0f;
         yr3=tthetaf*fZfpp[5]+yatz0f;
         xf4=tphia*fZfpp[11]+xinit;
         yf4=tthetaa*fZfpp[11]+yinit;
         xr4=tphif*fZfpp[11]+xatz0f;
         yr4=tthetaf*fZfpp[11]+yatz0f;

         dcone3=sqrt(pow((xf3-xr3),2)+pow((yf3-yr3),2));
         dcone4=sqrt(pow((xf4-xr4),2)+pow((yf4-yr4),2));

      ttheta=tan(theta_az_frvdc*3.14159265/180.0);
      zback=fZfpp[5];
      r1x3 = (zback-zclose_frvdc)*(tphia + (ttheta-tphia)/(1.0+ttheta*tphia));
      r2x3 = (zback-zclose_frvdc)*((ttheta+tphia)/(1.0-ttheta*tphia) - tphia);
      r1y3 = (zback-zclose_frvdc)*(tthetaa +(ttheta-tthetaa)/(1.0+ttheta*tthetaa));
      r2y3 = (zback-zclose_frvdc)*((ttheta+tthetaa)/(1.0-ttheta*tthetaa)- tthetaa);

      zback=fZfpp[11];
      r1x4 = (zback-zclose_frvdc)*(tphia + (ttheta-tphia)/(1.0+ttheta*tphia));
      r2x4 = (zback-zclose_frvdc)*((ttheta+tphia)/(1.0-ttheta*tphia) - tphia);
      r1y4 = (zback-zclose_frvdc)*(tthetaa +(ttheta-tthetaa)/(1.0+ttheta*tthetaa));
      r2y4 = (zback-zclose_frvdc)*((ttheta+tthetaa)/(1.0-ttheta*tthetaa)- tthetaa);

       inside1=true;
       inside2=true;
       xpt1[0]=xf3+r1x3;
       xpt1[1]=xf3+r1x3/sqrt(2.);
       xpt1[2]=xf3;
       xpt1[3]=xf3-r2x3/sqrt(2.);
       xpt1[4]=xf3-r2x3;
       xpt1[5]=xf3-r2x3/sqrt(2.);
       xpt1[6]=xf3;
       xpt1[7]=xf3+r1y3/sqrt(2.);
       ypt1[0]=yf3;
       ypt1[1]=yf3+r1y3/sqrt(2.);
       ypt1[2]=yf3+r1y3;
       ypt1[3]=yf3+r1y3/sqrt(2.);
       ypt1[4]=yf3;
       ypt1[5]=yf3-r2y3/sqrt(2.);
       ypt1[6]=yf3-r2y3;
       ypt1[7]=yf3-r2y3/sqrt(2.);

       xpt2[0]=xf4+r1x4;
       xpt2[1]=xf4+r1x4/sqrt(2.);
       xpt2[2]=xf4;
       xpt2[3]=xf4-r2x4/sqrt(2.);
       xpt2[4]=xf4-r2x4;
       xpt2[5]=xf4-r2x4/sqrt(2.);
       xpt2[6]=xf4;
       xpt2[7]=xf4+r1y4/sqrt(2.);
       ypt2[0]=yf4;
       ypt2[1]=yf4+r1y4/sqrt(2.);
       ypt2[2]=yf4+r1y4;
       ypt2[3]=yf4+r1y4/sqrt(2.);
       ypt2[4]=yf4;
       ypt2[5]=yf4-r2y4/sqrt(2.);
       ypt2[6]=yf4-r2y4;
       ypt2[7]=yf4-r2y4/sqrt(2.);

       for(Int_t k=0; k < 8; k++){
          j=1;
          oddnodes1=-1;
                                                                               
          for(Int_t i=0; i < nsides1; i++){
            j=j+1;
            if(j == (nsides1+1)) {j=1;}
            if(((ypoly1[i] < ypt1[k])&&(ypoly1[j-1]>=ypt1[k]))||
             ((ypoly1[j-1] < ypt1[k])&&(ypoly1[i] >= ypt1[k]))){
                  if((xpoly1[i]+((ypt1[k]-ypoly1[i])/(ypoly1[j-1]-ypoly1[i])*
                    (xpoly1[j-1]-xpoly1[i]))) < xpt1[k]){
                          oddnodes1= -1*oddnodes1;
                  }
            }
          }
          ...
          for(Int_t i=0; i < nsides2; i++){
             j=j+1;
             if(j == (nsides2+1)){j=1;}
             if(((ypoly2[i] < ypt2[k])&&(ypoly2[j-1]>=ypt2[k]))||
               ((ypoly2[j-1] < ypt2[k])&&(ypoly2[i]>=ypt2[k]))) {
                  if((xpoly2[i]+((ypt2[k]-ypoly2[i])/(ypoly2[j-1]-ypoly2[i])*
                     (xpoly2[j-1]-xpoly2[i]))) < xpt2[k]){
                     oddnodes2=-1*oddnodes2;
                  }
              }
          }
    ...
    // mkj crude front start section
	   // start Ed Brash conetest definition for VDC vs crude front
        // 
         xf3=tphia*fZfpp[5]+xinit;
         yf3=tthetaa*fZfpp[5]+yinit;
         xr3=tphif_cr*fZfpp[5]+xatz0f_cr;
         yr3=tthetaf_cr*fZfpp[5]+yatz0f_cr;
         xf4=tphia*fZfpp[11]+xinit;
         yf4=tthetaa*fZfpp[11]+yinit;
         xr4=tphif_cr*fZfpp[11]+xatz0f_cr;
         yr4=tthetaf_cr*fZfpp[11]+yatz0f_cr;

         dcone3=sqrt(pow((xf3-xr3),2)+pow((yf3-yr3),2));
         dcone4=sqrt(pow((xf4-xr4),2)+pow((yf4-yr4),2));

      ttheta=tan(theta_az_cfrvdc*3.14159265/180.0);
      zback=fZfpp[5];
      r1x3 = (zback-zclose_cfrvdc)*(tphia + (ttheta-tphia)/(1.0+ttheta*tphia));
      r2x3 = (zback-zclose_cfrvdc)*((ttheta+tphia)/(1.0-ttheta*tphia) - tphia);
      r1y3 = (zback-zclose_cfrvdc)*(tthetaa +(ttheta-tthetaa)/(1.0+ttheta*tthetaa));
      r2y3 = (zback-zclose_cfrvdc)*((ttheta+tthetaa)/(1.0-ttheta*tthetaa)- tthetaa);

      zback=fZfpp[11];
      r1x4 = (zback-zclose_cfrvdc)*(tphia + (ttheta-tphia)/(1.0+ttheta*tphia));
      r2x4 = (zback-zclose_cfrvdc)*((ttheta+tphia)/(1.0-ttheta*tphia) - tphia);
      r1y4 = (zback-zclose_cfrvdc)*(tthetaa +(ttheta-tthetaa)/(1.0+ttheta*tthetaa));
      r2y4 = (zback-zclose_cfrvdc)*((ttheta+tthetaa)/(1.0-ttheta*tthetaa)- tthetaa);
...
// and track.
         ...
         th_az=theta_az;
         ph_az=phi_az;
         th_az_frvdc=theta_az_frvdc;
         ph_az_frvdc=phi_az_frvdc;
         th_az_cfrvdc=theta_az_cfrvdc;
         ph_az_cfrvdc=phi_az_cfrvdc;
 ...
 //         evdsp << nr_interrupts << endl;
      ...
      else if  ( ntrack_front == 2 && n_track >= 1 && ana_front) {
	// add code if just front fpp and VDC
           xatz0f=(track[0][0]-track[0][2])/1.414213562;
           yatz0f=(track[0][0]+track[0][2])/1.414213562;
           fg = 180.00000/3.141592653589793;
           tthetaf = (track[0][1]+track[0][3])/1.414213562;
...
// decided to put a 10cm "border" around the outside, to be conservative
	   ...
	   for(Int_t k=0; k < 8; k++){
	     ...
	     for(Int_t i=0; i < nsides1; i++){
	       j=j+1;
	       if(j == (nsides1+1)) {j=1;}
	       if(((ypoly1[i] < ypt3[k])&&(ypoly1[j-1]>=ypt1[k]))||
		  ((ypoly1[j-1] < ypt3[k])&&(ypoly1[i] >= ypt1[k]))){
		 if((xpoly1[i]+((ypt3[k]-ypoly1[i])/(ypoly1[j-1]-ypoly1[i])*
				(xpoly1[j-1]-xpoly1[i]))) < xpt3[k]){
		   oddnodes1= -1*oddnodes1;
		 }
	       }
	     }
	     ...
	     for(Int_t i=0; i < nsides2; i++){
	       j=j+1;
	       if(j == (nsides2+1)){j=1;}
	       if(((ypoly2[i] < ypt4[k])&&(ypoly2[j-1]>=ypt4[k]))||
		  ((ypoly2[j-1] < ypt4[k])&&(ypoly2[i]>=ypt4[k]))) {
		 if((xpoly2[i]+((ypt4[k]-ypoly2[i])/(ypoly2[j-1]-ypoly2[i])*
				(xpoly2[j-1]-xpoly2[i]))) < xpt4[k]){
		   oddnodes2=-1*oddnodes2;
		 }
	       }
	     }
