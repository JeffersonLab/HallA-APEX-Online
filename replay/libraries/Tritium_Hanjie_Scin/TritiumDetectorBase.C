...
TritiumDetectorBase::TritiumDetectorBase( const char* name,
  ...
  fXax(1.0,0.0,0.0), fYax(0.0,1.0,0.0), fZax(0.0,0.0,1.0)
{
  ...
  fDetMap2 = new TritiumDetMap;
}

//_____________________________________________________________________________
TritiumDetectorBase::TritiumDetectorBase() : fDetMap2(0) {
  // for ROOT I/O only
}

//_____________________________________________________________________________
TritiumDetectorBase::~TritiumDetectorBase()
{
  // Destructor
  delete fDetMap2;
}
...
//_____________________________________________________________________________
void TritiumDetectorBase::DefineAxes( Double_t rotation_angle )
{
...
//_____________________________________________________________________________
Int_t TritiumDetectorBase::FillDetMap( const vector<Int_t>& values, UInt_t flags,
				   const char* here )
{
  // Utility function to fill this detector's detector map.
  // See TritiumDetMap::Fill for documentation.

  Int_t ret = fDetMap2->Fill( values, flags );
  if( ret == 0 ) {
    Error( Here(here), "No detector map entries found. Check database." );
  } else if( ret == -1 ) {
    Error( Here(here), "Too many detector map entries (maximum %d)",
	   TritiumDetMap::kDetMapSize );
  } else if( ret < -1 ) {
...
//_____________________________________________________________________________
Bool_t TritiumDetectorBase::IsInActiveArea( Double_t x, Double_t y ) const
{
...
//_____________________________________________________________________________
void TritiumDetectorBase::PrintDetMap( Option_t* opt ) const
{
...
//_____________________________________________________________________________
Int_t TritiumDetectorBase::ReadGeometry( FILE* file, const TDatime& date,
				     Bool_t required )
{
