...
class TritiumDetectorBase : public THaAnalysisObject {

public:
  virtual ~TritiumDetectorBase();

  TritiumDetectorBase(); // only for ROOT I/O

  virtual Int_t    Decode( const THaEvData& ) = 0;

  TritiumDetMap*       GetDetMap() const { return fDetMap2; }
...
protected:
  ...
  // Mapping
  TritiumDetMap *fDetMap2;    // Hardware channel map for this detector
  ...
  TritiumDetectorBase( const char* name, const char* description );

  ClassDef(TritiumDetectorBase,2)   //ABC for a detector or subdetector
};
