#ifndef ROOT_TritiumDetMap
#define ROOT_TritiumDetMap
...
public:
  ...
  TritiumDetMap();
  TritiumDetMap( const TritiumDetMap& );
  TritiumDetMap& operator=( const TritiumDetMap& );
  virtual ~TritiumDetMap();
...
protected:
  ...
  ClassDef(TritiumDetMap,0)   //The standard detector map
};

inline TritiumDetMap::Module* TritiumDetMap::GetModule( UShort_t i ) const {
  return i<fNmodules ? uGetModule(i) : NULL; 
}

inline Bool_t TritiumDetMap::IsADC(Module* d) {
  if( !d ) return kFALSE;
  return d->IsADC();
}

inline Bool_t TritiumDetMap::IsTDC(Module* d) {
  if( !d ) return kFALSE;
  return d->IsTDC();
}

inline UInt_t TritiumDetMap::GetModel(Module* d) {
  if( !d ) return 0;
  return d->GetModel();
}

inline Bool_t TritiumDetMap::IsADC( UShort_t i ) const {
  if( i >= fNmodules ) return kFALSE;
  return uGetModule(i)->IsADC();
}

inline Bool_t TritiumDetMap::IsTDC( UShort_t i ) const {
  if( i >= fNmodules ) return kFALSE;
  return uGetModule(i)->IsTDC();
}

inline UInt_t TritiumDetMap::GetModel( UShort_t i ) const {
  if( i >= fNmodules ) return 0;
  return uGetModule(i)->GetModel();
}

inline Int_t TritiumDetMap::GetNchan( UShort_t i ) const {
