...
//   Tong Su,Sep,2017
//   To decode the scaler data who has the event type 1-14
//
//   To use in the analyzer, your setup script needs something like this
//       gHaEvtHandlers->Add (new Tritium_TSScaler("Left","HA scaler event type 1-14"));
//
//   To enable debugging you may try this in the setup script
// 
//     Tritium_TSScaler *lscaler = new Tritium_TSScaler("Left","HA scaler event type 1-14");
...
Tritium_TSScaler::~Tritium_TSScaler()
{
  delete [] rdata;
  if (fScalerTree) {
    delete fScalerTree;
...
Int_t Tritium_TSScaler::End( THaRunBase* r)
{
  if (fScalerTree) fScalerTree->Write();
  return 0;
}

Int_t Tritium_TSScaler::Analyze(THaEvData *evdata)
{
  Int_t lfirst=1;
  ...
  if (fDebugFile) {
    ...
    *fDebugFile << "\nEnter Tritium_TSScaler  for fName = "<<fName<<endl;
    EvDump(evdata);
  }
...
giveup1:
  ...
  // event type 1-14 and come here.  If you found no headers, it was
  ...
  for (size_t i = 0; i < scalerloc.size(); i++) {
    ...
    if( ivar < scalerloc.size() && idx < scalers.size() && ichan < MAXCHAN ) {
      ...
      cout << "Tritium_TSScaler:: ERROR:: incorrect index "<<ivar<<"  "<<idx<<"  "<<ichan<<endl;
    }
  }
...
THaAnalysisObject::EStatus Tritium_TSScaler::Init(const TDatime& date)
{
  const int LEN = 200;
  char cbuf[LEN];
  ...
  eventtypes.push_back(4);  // what events to look for
  eventtypes.push_back(5);
  eventtypes.push_back(6);
  eventtypes.push_back(7);  // what events to look for
  eventtypes.push_back(8);
  eventtypes.push_back(9);
  eventtypes.push_back(10);  // what events to look for
  eventtypes.push_back(11);
  eventtypes.push_back(12);
  eventtypes.push_back(13);
  eventtypes.push_back(14);  
...
// Parse the map file which defines what scalers exist and the global variables.
  ...
  while( fgets(cbuf, LEN, fi) != NULL) {
	...
	sscanf(sinput.c_str(),"%s %d %d %d %x %x %d \n",cdum,&imodel,&icrate,&islot, &header, &mask, &inorm);
        if (fNormSlot >= 0 && fNormSlot != inorm) cout << "Tritium_TSScaler:: WARN: contradictory norm slot "<<inorm<<endl;
...
// fNormSlot is the slot#, checked for consistency
	  if (clkchan >= 0) {  
             ...
             fNormIdx = idx;
             if (islot != fNormSlot) cout << "Tritium_TSScaler:: WARN: contradictory norm slot ! "<<islot<<endl;  
...
// Verify that the slots are not defined twice
  ...
  for (UInt_t i1=0; i1 < scalers.size()-1; i1++) {
	...
	cout << "Tritium_TSScaler:: WARN:  same slot defined twice"<<endl;
    }
  }
// Identify indices of scalers[] vector to variables.
  ...
  if(fDebugFile) {
    *fDebugFile << "Tritium_TSScaler:: Name of scaler bank "<<fName<<endl;
    for (UInt_t i=0; i<scalers.size(); i++) {
...
void Tritium_TSScaler::AddVars(TString name, TString desc, Int_t islot,
...
// Will put that in later.
  ScalerLoc4 *loc = new ScalerLoc4(name1, desc1, 0, islot, ichan, ikind);
  loc->ivar = scalerloc.size();  // ivar will be the pointer to the dvars array.
  scalerloc.push_back(loc);
}

void Tritium_TSScaler::DefVars()
{
  ...
  if(fDebugFile) *fDebugFile << "Tritium_TSScaler:: scalerloc size "<<scalerloc.size()<<endl;
  const Int_t* count = 0;
  for (UInt_t i = 0; i < scalerloc.size(); i++) {
