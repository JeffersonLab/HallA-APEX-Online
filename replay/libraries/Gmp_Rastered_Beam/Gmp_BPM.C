...
Gmp_BPM::Gmp_BPM( const char* name, const char* description,
...
//                         inconsistent data
//
Int_t Gmp_BPM::ReadDatabase( const TDatime& date )
{
  ...
  do {
    ...
    printf("name :  %s \n",GetName());
    printf("BPM: Read in database --> %d %d %d %d %d %d \n",first_chan,crate,slot,first,last,modulid);
    if (first_chan>=0) {
  ...
  printf("Rotation  %f %f %f %f    Offsets %f %f \n",dummy1,dummy2,dummy3,dummy4,dummy5,dummy6);
...
//_____________________________________________________________________________
Int_t Gmp_BPM::DefineVariables( EMode mode )
{
...
//_____________________________________________________________________________
Int_t Gmp_BPM::Decode( const THaEvData& evdata )
{
  ...
  if (loc_test) {
    cout << endl << "Channels on ROC1 SLOT 22 -- (1st channel is index 0)"<<endl;
    for (int ic = 0; ic < 16; ic++) {
      cout << "  channel "<<ic<<"   data = "<<evdata.GetData(1,22,ic,0)<<endl;
    }
  }
  */
  ClearEvent();
  ...
  //cout << fCorSignal(0) << fCorSignal(1) << endl;
...
Int_t Gmp_BPM::Process( )
{
  ...
  double raw_sum = 0;  //tmp
  for (Int_t k=0; k<4; k++)  //tmp
    raw_sum += fRawSignal(k);  //tmp

  if ( raw_sum > 15500 ){  //tmp
  //if ( raw_sum > 20000 ){  //tmp, absurdly high
    //this cut causes all the <TVector3::SetMag>: zero vector can't be stretched warnings
  ...
  fPosition.SetXYZ(
    ...
    //this causes all the <TVector3::SetMag>: zero vector can't be stretched warnings
    fPosition.SetXYZ(  //tmp
		     //-1000,  //tmp
		     //-1000,  //tmp
		     //-1  //tmp
		     -raw_sum*0.1,  //tmp
