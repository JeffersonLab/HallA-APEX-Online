void cerL(Int_t run, Int_t flag){
  // Int_t run,flag;    
  ...
  myfile.open("../output/cer_ped_L.txt",ios::out|ios::app);
  myfile<<run<<"   "<<flag<<"   ";
  
  myfile2.open("../output/cer_peak_L.txt",ios::out|ios::app);
  myfile2<<run<<"   "<<flag<<"   ";
  ...
  for(i=0;i<10;i++){
    ...
    gPad->SetLogy();
    if (flag==0)
      T->Draw(Form("FbusL.cer.a[%d]>>h%d",i,i),cut);
    if (flag==1)
      T->Draw(Form("L.cer.a[%d]>>h%d",i,i),cut);
  ...
  if(flag==1) c1->Print(Form("../plots/cerL_%d.pdf",run));
  if(flag==0) c1->Print(Form("../plots/cerFbusL_%d.pdf",run));
}
