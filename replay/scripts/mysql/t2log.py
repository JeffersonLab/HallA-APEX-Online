# get halog entry info for Tritium experiment
# Shujie Li, 2018.06
...
# flag: start / end
# runnum: in str
def read_log(runnum, flag, verb=1):
  ...
  url += "&author="+author
  response = requests.get(url) #-- don't need this on a CUE level machine
  response = response.json()
  if response['data']['currentItems']==0:
    return '0'
  loglink  = prefix+response['data']['entries'][0]['lognumber']
  if verb==0:
    return loglink 
  print(flag+" of run log entry:", loglink)
  ...
  return ctime,content,loglink

def read_online_url(runnum, verb = 1):
  ...
  try:
    ...
    if verb==0:
      return loglink
    print( response['data']['entries'][0]['title']+":",loglink)
