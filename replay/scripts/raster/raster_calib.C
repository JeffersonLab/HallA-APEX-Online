...
void raster_calib(int run){
  ...
  while(!gSystem->AccessPathName(Form("/volatile/halla/triton/tjhague/rfile_rast/coinc_test_%d_%d.root",run,i),kFileExists)){
    rootfile->Add(Form("/volatile/halla/triton/tjhague/rfile_rast/coinc_test_%d_%d.root",run,i));
  ...
  //Hardcoded numbers are the position of BPMB w.r.t. the target and the distance between the twoBPMs
  ...
  TCut beamcut = "TMath::Abs(" + arm + ".BPMA.x)<100 && N.evleft.current_d3>5.";
  ...
  cout << arm << ".URaster.raw2posA = " << UbpmAx_offset << " " << UbpmAy_offset << " " << UbpmAx_slope << " " << UbpmAy_slope << " 0.0 0.0" << endl; 
  cout << arm << ".URaster.raw2posB = " << UbpmBx_offset << " " << UbpmBy_offset << " " << UbpmBx_slope << " " << UbpmBy_slope << " 0.0 0.0" << endl; 
  cout << arm << ".URaster.raw2posT = " << Utargx_offset << " " << Utargy_offset << " " << Utargx_slope << " " << Utargy_slope << " 0.0 0.0" << endl << endl; 
  
  cout << arm << ".DRaster.raw2posA = " << DbpmAx_offset << " " << DbpmAy_offset << " " << DbpmAx_slope << " " << DbpmAy_slope << " 0.0 0.0" << endl; 
  cout << arm << ".DRaster.raw2posB = " << DbpmBx_offset << " " << DbpmBy_offset << " " << DbpmBx_slope << " " << DbpmBy_slope << " 0.0 0.0" << endl; 
  cout << arm << ".DRaster.raw2posT = " << Dtargx_offset << " " << Dtargy_offset << " " << Dtargx_slope << " " << Dtargy_slope << " 0.0 0.0" << endl; 
}
