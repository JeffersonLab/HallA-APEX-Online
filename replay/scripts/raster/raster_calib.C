...
void raster_calib(int run){
  ...
  while(!gSystem->AccessPathName(Format("/volatile/halla/triton/tjhague/rfile_rast/tritium_%d_%d.root",run,i),kFileExists)){
    rootfile->Add(Format("/volatile/halla/triton/tjhague/rfile_rast/tritium_%d_%d.root",run,i));
