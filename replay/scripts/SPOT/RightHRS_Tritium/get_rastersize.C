...
Int_t get_rastersize(TString codafname,TString runNo, Int_t firsteve, Int_t lasteve, TString rootfname)
{
 ...
 // THaApparatus* BEAM1 = new THaUnRasteredBeam("Lurb","Unraster Beamline");
  THaApparatus* FbusRrb = new THaRasteredBeam("FbusRrb","Raster Beamline"); 
  FbusRrb->AddDetector( new THaRaster("Raster2","Downstream raster") );
  FbusRrb->AddDetector( new THaBPM("BPMA","bpmA for raster beam"));
  FbusRrb->AddDetector( new THaBPM("BPMB","bpmB for raster beam"));
...
TH2F *fdrastraw_x_bpmb_y; TritiumSpot->GetObject("fdrastraw_x_bpmb_y", fdrastraw_x_bpmb_y);
  ...
  TH2F *haxis = new TH2F("haxis","Fast Raster 1 X vs.Y",200,5000,10000,200,5000,10000);
  haxis->Draw();
  ...
  c2A->cd(1);
  // Added to fix axes, show raster x-y in full window
  TH2F *haxis2 = new TH2F("haxis","Fast Raster 2 X vs.Y",20,5000,10000,200,5000,10000);
  haxis2->Draw();
...
//// BELOW IS FOR FADC FADC FADC --THREE TIMES FOR EVERYTHING IMPORTANT////
  ...
  fc2->cd(1);
  // Added to fix axes, show raster x-y in full window
  TH2F *fhaxis = new TH2F("fhaxis","FADC Fast Raster 1 X vs.Y",500,65000,75000,500,65000,75000);
  fhaxis->Draw();
  ...
  fc2A->cd(1);
  // Added to fix axes, show raster x-y in full window
  TH2F *fhaxis2 = new TH2F("fhaxis","FADC Fast Raster 2 X vs.Y",500,65000,75000,500,65000,75000);
  fhaxis2->Draw();
