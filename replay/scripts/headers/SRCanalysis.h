#ifndef SRCanalysis_H    // To make sure you don't declare the function more than once by including the header multiple times.
#define SRCanalysis_H
#include "rootalias.h"
#include "SQLanalysis.h"

TString  listdir    = Form("/w/halla-scifs17exp/triton/SRC_elist/"); 
TString  plotdir    = listdir+"plots/";
Int_t    maxpeaks   = 6; // maximum number of currents allowed in one run. 


//Int_t    good_electron(Int_t, Int_t = 5, Int_t = 1); // total electrons and raw counts per bin
...
// get info from SQL database
  ...
  c1->Print(listdir+plotdir+Form("bcm_%d.png",runnum));
