void CerPedR(Int_t run, Int_t flag){
  // Int_t run,flag;    
  ...
  for(i=0;i<10;i++){
    c1->cd(i+1);
    if (flag==0)
      T->Draw(Form("FbusR.cer.a_c[%d]>>h%d",i,i),cut);
    if (flag==1)
      T->Draw(Form("R.cer.a_c[%d]>>h%d",i,i),cut);
    ...
    if(flag==0)
      dr = Form("FbusR.cer.a_c[%d]",i);
    if(flag==1)
      dr = Form("R.cer.a_c[%d]",i);
    dr = dr + ">>";
    dr = dr + name;
    T->Draw(dr,cut);
    //fit SPE
