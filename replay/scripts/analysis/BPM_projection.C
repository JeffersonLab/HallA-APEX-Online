...
void BPM_projection(int Run=0, int first=0, int last=25000, TString type = "", char arm ='L'){


  string exp = getenv("EXPERIMENT");
  const TString rootfilePath = Form("/chafs1/work1/%s/Rootfiles/",exp.c_str());  
  ...
  if(Run==-1){cout << "Thank you, come again " <<endl; exit(1);}
    ...
    TString fname = Form("%s%s_online_%d.root",rootfilePath.Data(),exp.c_str(),Run);
	...
	TH1F *BPMA_X = new TH1F("BPMA_X",Form("BPMA x pos for %c arm, for run %d",arm,Run),200,-0.01,0.01);
	TH1F *BPMB_X = new TH1F("BPMB_X",Form("BPMB x pos for %c arm, for run %d",arm,Run),200,-0.01,0.01);
	TH1F *BPMA_Y = new TH1F("BPMA_Y",Form("BPMA y pos for %c arm, for run %d",arm,Run),200,-0.01,0.01);
	TH1F *BPMB_Y = new TH1F("BPMB_Y",Form("BPMB y pos for %c arm, for run %d",arm,Run),200,-0.01,0.01);
	TH1F *Target_x = new TH1F("Target_X",Form("Target x projected for %c arm, for run %d",arm,Run),200,-0.01,0.01);
	TH1F *Target_y = new TH1F("Target_Y",Form("Target y projected for %c arm, for run %d",arm,Run),200,-0.01,0.01);
		
	TH2F *Target_xy = new TH2F("Target_xy",Form("Target xy projected for %c arm, for run %d",arm,Run),200,-0.01,0.01,200,-0.01,0.01);
	TH2F *BPMA_XY = new TH2F("BPMA_xy",Form("BPMA XY pos for %c arm, for run %d",arm,Run),200,-0.005,0.005,200,-0.01,0.01);
	TH2F *BPMB_XY = new TH2F("BPMB_xy",Form("BPMB XY pos for %c arm, for run %d",arm,Run),200,-0.005,0.005,200,-0.01,0.01);
	...
	TCanvas *POS = new TCanvas;
		...
		gPad->SetGridx();
		gPad->SetGridy();
		T->Draw(Form("%s%crb.BPMA.y:%s%crb.BPMA.x>>BPMA_xy",type.Data(),arm,type.Data(),arm),Form("Entry$>=%d&&Entry$<=%d",first,last),"colz");
		BPMA_XY->GetXaxis()->SetTitle("X pos(m)");
		BPMA_XY->GetYaxis()->SetTitle("Y pos(m)");
		POS_2D->cd(2);
		gPad->SetGridx();
		gPad->SetGridy();
		T->Draw(Form("%s%crb.BPMB.y:%s%crb.BPMB.x>>BPMB_xy",type.Data(),arm,type.Data(),arm),Form("Entry$>=%d&&Entry$<=%d",first,last),"colz");
		...
		//double Ax_max =	BPMA_xy->GetXaxis()->GetLastBin
