...
// Initialize plots (xscaler style)
  ...
  TGLayoutHints *fLayout = new TGLayoutHints(kLHintsCenterX | kLHintsExpandX, 10, 10, 7, 7);
  ...
  gClient->GetColorByName("blue", gcolor);
  ...
  for (Int_t ipage = 0; ipage < npages; ipage++) {
    ...
    for (int row = 0; row < nrow; row++) { // rows 
	  ...
	  const TGFont *bfont = gClient->GetFont("-*-times-medium-r-normal-*-25-*-*-*-*-*-*-*"); //Change font size to change button size.
	  if (!bfont)
